...
SID=$(echo y | hcloud ECS NovaListServers |grep id\" | sed -n 's/^ *"id": "\(.*\)".*/\1/p')
echo y | hcloud ecs NovaAssociateSecurityGroup --cli-region=ap-southeast-1  --addSecurityGroup.name=migrate  --server_id=$SID
done <<< "$input"
